...
const DEFAULT_COPY: SectionCopyBlock = {
  eyebrow: 'Álbum de Momentos',
  title: 'Galeria de Memórias',
  description: 'Instantes que se transformaram em lembranças importantes e merecem ser revisitados.',
};

export function MemoryGallery({ items, isInsideSimulator = false, copy = DEFAULT_COPY }: MemoryGalleryProps) {
  ...
  return (
    <section className="py-12 px-4 sm:px-6 max-w-6xl mx-auto w-full" aria-labelledby="gallery-heading">
      <div className="text-center max-w-2xl mx-auto mb-10">
        ...
        <h2 id="gallery-heading" className="font-serif text-3xl sm:text-4xl text-brand-wine tracking-tight">
          {copy.title}
        </h2>
        ...
        {items.map((item, index) => {
          ...
          return (
            <motion.div
              ...
              className={`group relative rounded-2xl overflow-hidden bg-white p-2 border border-brand-rose/30 shadow-xs hover:shadow-md transition-all duration-300 cursor-pointer active:scale-[0.98] ${isSpanRow ? "sm:col-span-2" : ""}`}
            ...
            >
              ...
              {item.caption && <p className="text-xs font-serif text-brand-wine mt-2 px-1 text-center truncate">{item.caption}</p>}
            </motion.div>
          );
        })}
      </div>
